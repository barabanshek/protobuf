...
class ScatterGather {
 public:
  ...
  int Init() {
    ...
    desc_pool_ = reinterpret_cast<hw_desc*>(
        aligned_alloc(64, sizeof(hw_desc) * n_desc_));
    if (desc_pool_ == nullptr) return -1;
    comp_pool_ = reinterpret_cast<completion_record*>(
        aligned_alloc(32, sizeof(completion_record) * n_desc_));
    if (comp_pool_ == nullptr) return -1;

    memset(desc_pool_, 0, sizeof(hw_desc) * n_desc_);
    memset(comp_pool_, 0, sizeof(completion_record) * n_desc_);

    // Pre-setup descriptors.
    for (size_t i = 0; i < n_desc_; ++i) {
      desc_pool_[i].completion_addr =
          reinterpret_cast<uint64_t>(&comp_pool_[i]);
      desc_pool_[i].flags = dflags_;
      desc_pool_[i].opcode = DSA_OPCODE_MEMMOVE;
    }

    desc_batch_->completion_addr = reinterpret_cast<uint64_t>(compl_batch_);
    desc_batch_->flags = dflags_;
 ...
 private:
  static constexpr int dflags_ = IDXD_OP_FLAG_CRAV | IDXD_OP_FLAG_RCR;
