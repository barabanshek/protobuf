...
benchmark_path = protobuf_path + 'tachidromos/throughput_benchmark/'
benchmark_tmpl_path = benchmark_path + "src/reqsim.cc"
benchmark_source_path = benchmark_path + "src/reqsim_src.cc"
...
if args.run:
    # Run benchmark
    res = subprocess.run(f'./test', cwd=benchmark_path + 'build/', shell=True, text=True, capture_output=True)
    result = res.stdout
    print(result)
    if res.returncode != 0:
