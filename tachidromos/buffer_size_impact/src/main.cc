...
#define SCATTER_GATHER_BUFFER_SIZE 1024
...
int main () {
    ...
    for (size_t i = 0; i < kNofIterations; ++i) {
        ...
        //outcome = compress_with_IAA(gather_outs[i].data(), out_size, compressed[i], ACTUAL_BUFFER_SIZE, &comprOutputSize[i]);
        end = std::chrono::steady_clock::now();
    ...
    for (size_t i = 0; i < messages.size(); ++i) {
        ...
        //auto outcome = decompress_with_IAA(compressed[i], comprOutputSize[i], decompressed[i], ACTUAL_BUFFER_SIZE, &decomprOutputSize[i]);
        end = std::chrono::steady_clock::now();
