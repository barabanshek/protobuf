...
benchmark_path = protobuf_path + 'tachidromos/buffer_size_impact/'
benchmark_tmpl_path = benchmark_path + "src/main.cc"
#benchmark_tmpl_path = benchmark_path + "src/only_buffers.cc"
...
parser.add_argument('-bf', '--buffer-size', type=int, default=1024, help='(de)compression buffer size.')
...
if args.hacky:
    ...
    for j, line in enumerate(lines):
        if "<------------ BUFFER SIZE ------>" in line:
            ...
            lines.insert(j + 1, f'#define BUFFER_SIZE {args.buffer_size}\n')
            break
    for j, line in enumerate(lines):
        if "<------------ SETTERS ------>" in line:
            lines.insert(j + 1, setters)
